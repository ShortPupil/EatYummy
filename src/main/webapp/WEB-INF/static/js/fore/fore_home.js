...
$(function () {
    ...
    $(".banner_main > a").attr("href", "/food/" + $("#banner1").attr("name"));
    //单击商品分类链接时
    ...
    $(".banner_nav>li").hover(function () {
        ...
        $.ajax({
            type: "GET",
            url: "food/nav/" + $(this).attr("data-toggle"),
            data: null,
            dataType: "json",
            success: function (data) {
                if (data.success) {
                    var list = data.restaurant.complexFoodList;
                    for (var i = 0; i < list.length; i++) {
                        ...
                        for (var j = 0; j < list[i].length; j++) {
                            var food_title = list[i][j].food_title;
                            var index = food_title.indexOf(' ');
                            if (index !== -1) {
                                food_title = food_title.substring(0, index);
                            }
                            hot_word_div.append("<a href='food/" + list[i][j].food_id + "'>" + food_title + "</a>");
                        }
                    }
                    //热词样式
    ...
    $('form').submit(function () {
        if ($(this).find("input[name='food_name']").val() === "") {
            alert("请输入关键字！");
            return false;
        }
    });
...
function getTimer() {
    ...
    $(".banner_main > a").attr("href", "/food/" + img.attr("name"));
    if (index === 1) {
        color = "#0F1322";
