    ...
    <script>
        ...
        $(function () {
            ...
            $("#btn_productOrder_refresh").click(function () {
                ...
                //清除排序样式
                var table = $("#table_foodorder_list");
            ...
            //点击table中的数据时
            $("#table_foodorder_list").find(">tbody>tr").click(function () {
                trDataStyle($(this));
            });
        });
        //获取订单数据
        function getData(object,url,dataObject) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    //显示订单统计数据
                    $("#productOrder_count_data").text(data.foodorderCount);
                    if (data.foodorderList.length > 0) {
                        for (var i = 0; i < data.foodorderList.length; i++) {
                            var foodOrderStatusClass;
                            var foodOrderStatusTitle;
                            var foodOrderStatus;
                            switch (data.foodorderList[i].status) {
                                case 1:
                                    foodOrderStatusClass = "td_chart";
                                    foodOrderStatusTitle = "购物车";
                                    foodOrderStatus = "购物车";
                                    break;
                                case 2:
                                    foodOrderStatusClass = "td_pay";
                                    foodOrderStatusTitle = "等待买家付款";
                                    foodOrderStatus = "等待买家付款";
                                    break;
                                case 3:
                                    foodOrderStatusClass = "td_wait";
                                    foodOrderStatusTitle = "等待买家确认";
                                    foodOrderStatus = "等待买家确认";
                                    break;
                                case 4:
                                    foodOrderStatusClass = "td_success";
                                    foodOrderStatusTitle = "交易成功";
                                    foodOrderStatus = "交易成功";
                                    break;
                                default:
                                    foodOrderStatusClass = "td_error";
                                    foodOrderStatusTitle = "交易关闭";
                                    foodOrderStatus = "交易关闭";
                                    break;
                            }
                            var id = data.foodorderList[i].id;
                            var foodOrder_receiver = data.foodorderList[i].receiver;
                            var foodOrder_phonenumber = data.foodorderList[i].phonenumber;
                            var foodOrder_paydate = data.foodorderList[i].payDate;
                            var foodOrder_confirmdate = data.foodorderList[i].confirmDate;
                            //显示用户数据
                            tbody.append(
                                "<tr>" +
                                "<td>" +
                                "   <input type='checkbox' class='cbx_select' id='cbx_productOrder_select_" + id + "'>" +
                                "   <label for='cbx_productOrder_select_" + id + "'></label>" +
                                "</td>" +
                                "<td title='" + foodOrder_receiver + "'>" + foodOrder_receiver + "</td>" +
                                "<td title='" + foodOrder_phonenumber + "'>" + foodOrder_phonenumber + "</td>" +
                                "<td title='" + foodOrder_paydate + "'>" + foodOrder_paydate + "</td>" +
                                "<td title='" + foodOrder_confirmdate + "'>" + foodOrder_confirmdate + "</td>" +
                                "<td>" +
                                "<span class='" + foodOrderStatusClass + "' title= '" + foodOrderStatusTitle + "'>" + foodOrderStatus + "</span></td>" +
                                "<td hidden class='order_id'>" + id + "</td></tr>");
                        }
                        //绑定事件
...
<div class="frm_div text_info">
    ...
    <div class="frm_group">
        ...
        <input class="frm_radio radio_productOrder_status" id="checkbox_productOrder_status_inCart" name="checkbox_productOrder_status" type="checkbox" value="1" checked>
        <label class="frm_label" id="lbl_productOrder_status_inCart" for="checkbox_productOrder_status_inCart" title="在购物车中">购物车</label>
        <input class="frm_radio radio_productOrder_status" id="checkbox_productOrder_status_waitPay" name="checkbox_productOrder_status" type="checkbox" value="2" checked>
        <label class="frm_label" id="lbl_productOrder_status_waitPay" for="checkbox_productOrder_status_waitPay" title="等待买家付款">待付款</label>
        <input class="frm_radio radio_productOrder_status" id="checkbox_productOrder_status_waitConfirm" name="checkbox_productOrder_status" type="checkbox" value="3" checked>
        ...
        <input class="frm_radio radio_productOrder_status" id="checkbox_productOrder_status_success" name="checkbox_productOrder_status" type="checkbox" value="4" checked>
        <label class="frm_label" id="lbl_productOrder_status_success" for="checkbox_productOrder_status_success" title="交易成功">交易成功</label>
        <input class="frm_radio radio_productOrder_status" id="checkbox_productOrder_status_close" name="checkbox_productOrder_status" type="checkbox" value="0" checked>
...
<div class="data_count_div text_info">
    ...
    <span>订单总数:</span>
    <span class="data_count_value" id="productOrder_count_data">${requestScope.foodorderCount}</span>
    <span class="data_count_unit">个</span>
</div>
<div class="table_normal_div">
    <table class="table_normal" id="table_foodorder_list">
    ...
    <c:forEach items="${requestScope.foodorderList}" var="foodorder">
        <tr>
            <td><input type="checkbox" class="cbx_select" id="cbx_productOrder_select_${foodorder.id}"><label for="cbx_productOrder_select_${productOrder.productOrder_id}"></label></td>
            <td title="${foodorder.receiver}">${foodorder.receiver}</td>
            <td title="${foodorder.phonenumber}">${foodorder.phonenumber}</td>
            <td title="${foodorder.payDate}">${foodorder.payDate}</td>
            <td title="${foodorder.confirmDate}">${foodorder.confirmDate}</td>
            <td>
                <c:choose>
                    <c:when test="${foodorder.status==1}">
                        <span class="td_await" title="购物车">购物车</span>
                    </c:when>
            ...
            <td hidden class="order_id">${foodorder.id}</td>
        </tr>
    </c:forEach>
    </tbody>
    </table>
